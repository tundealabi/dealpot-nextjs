...
const pointek = async (page) => {
    try {
        const response = await axios.get(`https://www.pointekonline.com/buy-electronics-in-nigeria-samsung-electronics-from-pointek/page/${page}`);
        let $ = cheerio.load(response.data);
        return pointekHtml($);
    } catch (error) {
        console.log(error.message);
...
const electronicsApi = async (pagination) => {
