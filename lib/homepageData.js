...
const testWeb = async () => {
    try {
        const response = await axios.get("https://dealpot-test-web.netlify.app/headphone1.html");
        let $ = cheerio.load(response.data);
        if($(".info").find("h3").text().length > 3){
            return [
                {
                    vendor: "Testweb",
                    itemName: $(".detail").find("h3").first().text(),
                    itemPrice: $(".detail").find("p").first().text(),
                    itemImage: $(".info").find("img").attr("src"),
...
const homepageTestData = async () => await testWeb();

export { homepageDataApi, homepageTestData };